...
describe ActiveRecord::ConnectionAdapters::DepartureAdapter do
  shared_examples_for ActiveRecord::ConnectionAdapters::DepartureAdapter do |adapter_class, result_class|
    let(:mysql_client) { double(:mysql_client, server_info: { version: '5.7.19' }) }
    let(:mysql_adapter) do
      instance_double(adapter_class, raw_connection: mysql_client)
    end
    ...
    it { is_expected.to be_supports_migrations }
    ...
    describe '#new_column' do
      ...
      it do
        ...
        subject.new_column(field, default, type, null, table_name, default_function, collation, comment)
      end
    end
    ...
    describe '#exec_insert' do
      ...
      it 'executes the sql' do
        expect(subject).to(receive(:execute).with(sql, name))
        subject.exec_insert(sql, name, binds)
      end
    end
    ...
    describe '#select_rows' do
      ...
      let(:mysql_result) do
        instance_double(result_class, to_a: array_of_rows, fields: [:id, :body])
      end
    ...
    describe '#select' do
      ...
      let(:array_of_rows) { [%w[1 body], %w[2 body]] }
      let(:mysql_result) do
        instance_double(result_class, fields: %w[id body], to_a: array_of_rows)
      end
